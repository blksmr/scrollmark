...
- **Auto Overlay Detection** - Automatically detects sticky/fixed headers and adjusts scroll offset. No manual config needed.
- **Buttery Smooth** - RAF + throttling for 60fps performance. No jank, no polling, just smooth.
- **Hysteresis Scoring** - Smart algorithm prevents jittery switching between sections while scrolling.
- **Window & Container** - Works with both window scroll and custom scrollable containers.
- **Debug Mode** - Visual overlay showing scroll position, trigger line, and section scores.
- **TypeScript Ready** - Full type definitions included. Autocomplete everything.
